...
public class SecurityConfig  {
    ...
    public SecurityFilterChain config(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests(
                authorize->{
                    authorize.requestMatchers("/js/**", "/css/**", "/images/**", "/templates").permitAll(); // js, css, images로 가는 경로도 Security가 막아서 허용해줘야 함.
                    authorize.requestMatchers("/","/login").permitAll();
                    authorize.requestMatchers("/user/join").hasRole("ANONYMOUS");
                    authorize.requestMatchers("/user/myInfo").hasRole("USER"); // ROLE_USER
